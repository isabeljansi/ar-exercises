...
@mens_stores.each { |men_store| 
  puts "name: #{men_store.name} and annual revenue: #{men_store.annual_revenue}"
}

@womens_stores = Store.where(womens_apparel: true).where('annual_revenue < 1000000')

@womens_stores.each { |women_store| 
  puts "name: #{women_store.name} and annual revenue: #{women_store.annual_revenue }"
}
