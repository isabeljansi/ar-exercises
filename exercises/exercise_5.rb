...
puts "Total Revenue for the company: #{Store.sum(:annual_revenue)}"
puts "Average Annual Revenue for all stores: #{Store.average(:annual_revenue)}"
puts "Number of stores generating more than 1 Million in annual Sales: #{Store.where('annual_revenue > 1000000').count}"